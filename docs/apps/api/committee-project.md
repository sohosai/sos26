...
- `ownerId`: 企画メンバーであり、且つ他の有効な企画（`deletionStatus == null`）で責任者・副責任者に指定されていないユーザー
- `subOwnerId`: 企画メンバーであり、且つ他の有効な企画（`deletionStatus == null`）で責任者・副責任者に指定されていないユーザー（`null` で未設定）
...
- 指定したメンバーが他の有効な企画で責任者または副責任者として登録されている場合: `INVALID_REQUEST`
...
- 権限がない場合: `FORBIDDEN`
- 企画が存在しない場合: `NOT_FOUND`
- 削除状態を取り消す（`deletionStatus: null`）時に、この企画の責任者または副責任者が他の有効な企画で既に役割を持っている場合: `INVALID_REQUEST`
