...
### Phase 5: フロントエンド — 実委側 基本画面

17. **一覧画面の API 接続**: モックデータを `GET /committee/notices` に差し替え。テーブルのカラムを更新（`sharedUsers` → `collaborators`）
18. **作成ダイアログの API 接続**: `CreateNoticeDialog` を `POST /committee/notices` に接続
19. **お知らせ詳細画面（新規）**: `GET /committee/notices/:noticeId` で取得した内容を表示するページまたはダイアログ。RichTextEditor の出力（HTML）をレンダリング。編集・削除ボタンは権限に応じて表示制御（owner / 共同編集者のみ）
20. **編集ダイアログ**: 既存の `CreateNoticeDialog` を再利用または編集専用ダイアログを作成。`PATCH /committee/notices/:noticeId` に接続
21. **削除確認ダイアログ**: `DELETE /committee/notices/:noticeId` に接続。owner のみ操作可能

### Phase 6: フロントエンド — 実委側 共同編集者管理

22. **共同編集者セクション（詳細画面内）**: 現在の共同編集者一覧を表示。owner の場合は追加・削除 UI を表示
23. **共同編集者追加 UI**: 実委人の一覧から選択するセレクトまたはコンボボックス。`POST /committee/notices/:noticeId/collaborators` に接続
24. **共同編集者削除**: 各共同編集者の横に削除ボタン。`DELETE /committee/notices/:noticeId/collaborators/:collaboratorId` に接続

### Phase 7: フロントエンド — 実委側 配信承認フロー

25. **配信承認申請ダイアログ（新規）**: 以下を入力して `POST /committee/notices/:noticeId/authorizations` に送信
    - 承認者の選択（共同編集者の中から選択するセレクト）
    - 配信先企画の選択（複数選択可能なチェックボックスリスト）
    - 配信希望日時の入力（日時ピッカー）
26. **承認ステータス表示**: 詳細画面に承認状況（PENDING / APPROVED / REJECTED）を表示。配信先企画リストも表示
27. **承認・却下 UI**: 自分が `requestedTo` の PENDING 承認がある場合、承認 / 却下ボタンを表示。`PATCH /committee/notices/:noticeId/authorizations/:authorizationId` に接続
28. **配信状況ダイアログ（新規）**: `GET /committee/notices/:noticeId/status` で企画ごとの既読率（readCount / memberCount）を一覧表示

### Phase 8: フロントエンド — 企画側

29. **一覧画面の API 接続**: モックデータを `GET /project/:projectId/notices` に差し替え。`isRead` に基づき「未チェック / チェック済み」タグを表示
30. **詳細ダイアログの API 接続**: `GET /project/:projectId/notices/:noticeId` に差し替え。HTML 本文のレンダリング対応
