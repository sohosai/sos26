...
| PATCH | `/project/:projectId/detail` | 企画設定更新（企画責任者のみ） |
| POST | `/project/:projectId/invite-code/regenerate` | 企画参加コード再生成（企画責任者のみ） |
| GET | `/project/:projectId/members` | 企画メンバー一覧 |
| POST | `/project/:projectId/members/:userId/remove` | メンバー削除（企画責任者/副企画責任者） |
| POST | `/project/:projectId/members/:userId/assign` | 副企画責任者リクエストの送信（企画責任者のみ） |
| POST | `/project/:projectId/sub-owner-request/approve` | 副企画責任者リクエストを承認（依頼対象メンバー） |
| POST | `/project/:projectId/sub-owner-request/cancel` | 副企画責任者リクエストを取り消し（企画責任者のみ） |
| POST | `/project/:projectId/sub-owner-request/reject` | 副企画責任者リクエストを辞退（依頼対象メンバー） |
...
- `pendingSubOwnerRequestUserId` を返却
  - `PENDING` の副企画責任者リクエストがある場合は対象ユーザーID
  - なければ `null`
...
- 対象ユーザー宛ての `PENDING` の副企画責任者リクエストがあれば同時に `REJECTED` 化

### POST `/project/:projectId/members/:userId/assign`

企画メンバーへ「副企画責任者リクエスト」を送信します。
...
企画責任者が `PENDING` の副企画責任者リクエストを取り消します。
...
- OWNER
  - 設定変更、企画参加コード再生成、副企画責任者リクエストの送信が可能
- SUB_OWNER
  - メンバー削除が可能
- MEMBER
  - メンバー一覧の閲覧が可能
  - 自分宛の副企画責任者リクエストの承認/辞退が可能
