...
| `permissions` | `Set<CommitteePermission> \| null` | 保有する実委人権限の集合（ログイン直後の取得前は `null`、非委員でも空 Set） |
...
> ```
> `permissions` は `GET /auth/me` のレスポンスから設定されるため、追加の API 呼び出しなしで権限判定が可能（旧 `preloadMemberEditPermission` / `preloadProjectRegistrationPermission` は廃止された）。
...
| `getMe()` | GET /auth/me | 現在のユーザー・委員メンバー情報・保有権限一覧を取得（要認証） |
