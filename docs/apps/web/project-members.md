...
このドキュメントは、「企画メンバー管理」の仕様と実装方針をまとめたものです。
...
- [API 仕様（要点）](#api-仕様要点)
  - [一覧取得](#一覧取得)
  - [削除](#削除)
  - [副企画責任者リクエスト](#副企画責任者リクエスト)
  - [企画参加コード再生成](#企画参加コード再生成)
- [役職と制約](#役職と制約)
- [データ整合性（DB）](#データ整合性db)
...
企画参加者の運用に必要な以下の機能が整備されています。

1. 企画メンバー一覧の表示
2. 企画参加コードによる参加導線
3. メンバー削除（企画責任者/副企画責任者）
4. 副企画責任者リクエストの送信（企画責任者のみ）
5. 企画参加コード再生成（企画責任者のみ）
...
- SUB_OWNER が未設定のときのみ「副企画責任者に指名」を MEMBER の行に表示
- MEMBER 行には「削除」を表示
...
「メンバーを追加」ボタンから招待ダイアログを開き、企画参加コードを表示・コピーできます。
