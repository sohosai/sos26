...
実委人（委員会メンバー）が申請を作成し、承認フローを経て企画（プロジェクト）に配信する機能。企画メンバーは配信された申請に対して企画単位で回答を提出する。
...
| 用語 | 説明 |
|------|------|
| **申請（Form）** | 実委人が作成する質問票。タイトル・説明文・設問（FormItem）から構成される |
| **設問（FormItem）** | 申請内の1つの質問。タイプ・ラベル・必須フラグ・選択肢を持つ |
| **選択肢（FormItemOption）** | SELECT / CHECKBOX 設問の選択肢 |
| **共同編集者（Collaborator）** | 申請作成者以外で申請の編集・閲覧ができる実委人 |
| **閲覧者（FormViewer）** | 回答の閲覧権限を持つ実委人。スコープ（全員/局/個人）で指定する |
| **承認依頼（FormAuthorization）** | 申請配信の承認依頼。配信日時・締切・配信先企画を指定する |
...
- `ownerId` → User: 申請の作成者。削除・共同編集者管理はこのユーザーのみ
- `deletedAt`: 論理削除。`null` でない申請は一覧・詳細・企画側のいずれからも不可視
...
- 回答の閲覧権限を付与するための設定。Owner / Collaborator 以外にも回答閲覧を許可できる
- `scope`: 閲覧者の範囲を指定する `ViewerScope` enum

| scope | 説明 | 必要フィールド |
|-------|------|--------------|
| `ALL` | 全実委人が閲覧可能 | — |
| `BUREAU` | 指定した局の実委人が閲覧可能 | `bureauValue` |
| `INDIVIDUAL` | 指定した個人が閲覧可能 | `userId` |

- 更新時は既存レコードを全削除（物理削除）→新規作成で全置換する
...
- タイプに応じて `textValue` / `numberValue` / `fileId` のいずれかに値が入る
...
| 申請作成者（Owner） | `Form.ownerId` が自身のユーザー |
| 共同編集者（Collaborator） | `FormCollaborator` に登録されたユーザー。Owner と同等の編集権限を持つ |
| 閲覧者（Viewer） | `FormViewer` に登録された実委人。回答の閲覧のみ可能 |
| FORM_DELIVER 権限保持者 | `CommitteeMemberPermission` で `FORM_DELIVER` を持つ実委人 |
| 企画メンバー（Project Member） | 企画の owner / subOwner / member |

### 4.2 操作 × 権限マトリクス（実委人側）

| 操作 | Owner | Collaborator | Viewer | その他の実委人 | 状態制約 |
|------|:-----:|:----------:|:------:|:-------------:|---------|
| 申請作成 | — | — | — | 全員可 | — |
| 申請一覧取得 | — | — | — | 全員可 | 削除済み除外 |
| 申請詳細取得 | — | — | — | 全員可 | 削除済み除外 |
| 申請編集 | OK | OK | NG | NG | APPROVED な承認依頼が存在しないこと |
| 申請削除 | OK | NG | NG | NG | — |
| 共同編集者の追加 | OK | NG | NG | NG | — |
| 共同編集者の削除 | OK | NG | NG | NG | — |
| 閲覧者の設定 | OK | OK | NG | NG | — |
| 承認依頼を送信 | OK | OK | NG | NG | — |
...
全エンドポイントに `requireAuth` + `requireProjectMember` が必須。**自分が所属する企画宛に配信された申請のみ**が操作対象となる。

| 操作 | 企画メンバー | 追加条件 |
|------|:-----------:|---------|
| 申請一覧 | OK | 自企画宛の配信済み申請のみ（§5.5 表示条件参照） |
| 申請詳細 | OK | 同上 |
...
**バリデーション:**
- 同一申請に `PENDING` または `APPROVED` の承認依頼が既に存在しないこと
- 承認者が `FORM_DELIVER` 権限を持つ実委人であること（承認依頼者と同一ユーザーでもよい）
...
**承認時の追加バリデーション:**
- 申請が削除されていないこと（`form.deletedAt` が `null`）
...
- status が `REJECTED` の場合、申請は編集可能に戻る
- 修正後、新たに承認依頼を作成できる（新しい `FormAuthorization` レコード）

### 5.5 配信

承認後、申請は自動的に配信される。明示的な「配信」操作は不要。

**企画側で申請が表示される条件（全て AND）:**
1. `FormAuthorization.status` = `APPROVED`
2. `scheduledSendAt` <= 現在日時
3. `form.deletedAt` = `null`

この 3 条件を満たすと、該当する `FormDelivery` に紐づく企画の申請一覧に自動表示される。
...
| 承認後〜配信前（now < scheduledSendAt） | 企画側に申請は表示されない（SCHEDULED 状態） |
| 配信後（now >= scheduledSendAt） | 企画側に申請が表示される（PUBLISHED 状態） |
...
**例:**
> 企画メンバー A が下書きを保存 → メンバー B が申請を開くと A の下書きが表示される → B が編集・提出すると `respondentId` は B になる
...
| FILE | `fileId` が `null` または falsy（空文字含む） |
...
以下のいずれかに該当する実委人が閲覧可能:

1. **Owner** — 申請作成者
2. **Collaborator** — 共同編集者
3. **Viewer** — 閲覧者（`FormViewer`）にマッチする実委人
   - `scope = ALL`: 全実委人
   - `scope = BUREAU`: 該当局に所属する実委人
   - `scope = INDIVIDUAL`: 指定されたユーザー本人

上記いずれにも該当しない実委人は閲覧不可。
...
| メソッド | パス | 説明 | 権限 |
|---------|------|------|------|
| POST | `/create` | 申請作成（設問・選択肢含む一括登録） | 実委人 |
| GET | `/list` | 申請一覧取得（削除済み除外、最新承認情報付き） | 実委人 |
| GET | `/:formId/detail` | 申請詳細（設問・共同編集者・承認詳細付き） | 実委人 |
| PATCH | `/:formId/detail` | 申請編集（タイトル・説明・設問の CRUD） | Owner / Collaborator |
| DELETE | `/:formId` | 申請論理削除 | Owner |
...
| メソッド | パス | 説明 | 権限 |
|---------|------|------|------|
| PUT | `/:formId/viewers` | 閲覧者設定（全置換） | Owner / Collaborator |
...
| メソッド | パス | 説明 | 権限 |
|---------|------|------|------|
| GET | `/:formId/responses` | 提出済み回答一覧（回答者・企画・回答値付き） | Owner / Collaborator / Viewer |
| GET | `/:formId/responses/:responseId` | 提出済み回答の詳細取得 | Owner / Collaborator / Viewer |

### 10.2 企画側（`/project/:projectId/forms`）

| メソッド | パス | 説明 | 権限 |
|---------|------|------|------|
| GET | `/` | 配信済み申請一覧（回答状況付き） | 企画メンバー |
| GET | `/:formDeliveryId` | 申請詳細 + 既存回答（下書き含む） | 企画メンバー |
...
// FILE
{ formItemId: string, type: "FILE", fileId: string | null }
...
3. 既に APPROVED で配信済みでも、企画側からは申請が見えなくなる
