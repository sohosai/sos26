...
| 公開先企画 | チェックボックスリスト（検索・全選択機能付き） |

- 送信中はボタンを無効化しローディング表示
- API エラー時はエラーメッセージを表示し、ダイアログは閉じない（申請データを保持）
