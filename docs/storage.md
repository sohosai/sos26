...
ブラウザ                                                    S3
  ...
  │ ────────────────────────────────────────────────────>  │
  │  { previewUrl } (Presigned GET, inline)                │
  │ <────────────────────────────────────────────────────  │
  │                                                        │
  │ GET previewUrl                                         │
  │ ────────────────────────────────────────────────────>  │
...
動画・画像は **S3 Presigned GET URL（inline）で直接ストリーミング**する。API サーバーを介さないため負荷がゼロ。

#### 文書（PDF/DOCX/XLSX）（API プロキシ）
...
文書はブラウザでの直接表示が難しいため、**API プロキシで Blob 取得後にレンダリング**する。
...
ブラウザ                                                    S3
  ...
  │ ────────────────────────────────────────────────────>  │
  │  { downloadUrl } (Presigned GET, attachment)           │
  │ <────────────────────────────────────────────────────  │
  │                                                        │
  │ GET downloadUrl                                        │
  │ ────────────────────────────────────────────────────>  │
...
ダウンロードも **S3 Presigned GET URL（attachment）で直接取得**する。
...
ブラウザから Presigned URL で直接 PUT/GET するため、バケットに CORS 設定が必要。
さくらのオブジェクトストレージのコントロールパネルで、以下の値を設定する:

| 項目 | 値 |
|------|-----|
| AllowedOrigins | `http://localhost:5173`, `https://本番ドメイン`（`CORS_ORIGIN` と同じ） |
| AllowedMethods | `PUT`, `GET`（アップロード + プレビュー/ダウンロード） |
| AllowedHeaders | `Content-Type` |
| ExposeHeaders | `Content-Disposition`, `Content-Length`（ダウンロード/プレビュー用） |
| MaxAgeSeconds | `3600` |
