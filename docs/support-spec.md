...
| 件名 | o | テキスト入力 |
| 内容 | o | テキストエリア |
| 関連フォーム | - | 自分がオーナーまたは共同編集者のフォームから選択 |
| 対象企画 | o | 企画一覧から選択 |
...
### 5.1 フォームに関連するお問い合わせ（企画側作成時）

企画人が関連フォームを選択してお問い合わせを作成した場合:

| 対象 | 初期割り当て |
|------|------------|
| フォームオーナー | 実委側の初期担当者 → ステータスを `IN_PROGRESS` に。メール・プッシュ通知を送信 |
| フォームの共同編集者 | 初期の閲覧者（個人スコープ） |

- 初期割り当て後、担当者・閲覧者の変更は可能
- **兼任ユーザーの重複排除**: フォームオーナーや共同編集者が企画側担当者（作成者・共同担当者）と同一人物の場合、実委側担当者・閲覧者への自動追加から除外される（DB のユニーク制約 `[inquiryId, userId, deletedAt]` との整合性を保つため）
- フォームオーナーが除外された場合、初期ステータスは `UNASSIGNED` となる

### 5.2 フォームに関連するお問い合わせ（実委側作成時）

- 実委側はフォームの存在チェックのみ行い、初期対応ルール（自動担当者・閲覧者割り当て）は適用しない
- 作成者が実委側担当者となるため、ステータスは `IN_PROGRESS` で開始

### 5.3 フォームに関連しないお問い合わせ
...
- サイドバー: ステータス、担当者一覧、閲覧者設定（実委側のみ）、関連フォーム（実委側はリンク、企画側はテキスト表示）
...
- 企画側: 件名、内容、関連フォーム（配信されたフォームから選択）、共同担当者、添付ファイル
- 実委側: 件名、内容、関連フォーム（オーナーまたは共同編集者のフォームから選択）、対象企画、企画側担当者（必須）、追加実委担当者、閲覧者、添付ファイル
