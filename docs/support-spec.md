...
- **作成者（企画者が作成した場合）は担当者から外れることができない**
...
- 企画者は実委側の担当者を設定できない
- 初期ステータスは `UNASSIGNED`（関連申請による初期割り当てが行われた場合は `IN_PROGRESS`、5章参照）
...
### 5.1 申請に関連するお問い合わせ（企画側作成時）

企画者が関連申請を選択してお問い合わせを作成した場合:

| 対象 | 初期割り当て |
|------|------------|
| 申請オーナー | 実委側の初期担当者 → ステータスを `IN_PROGRESS` に。メール・プッシュ通知を送信 |
| 申請の共同編集者 | 初期の閲覧者（個人スコープ） |

- 初期割り当て後、担当者・閲覧者の変更は可能
- **兼任ユーザーの重複排除**: 申請オーナーや共同編集者が企画側担当者（作成者・共同担当者）と同一人物の場合、実委側担当者・閲覧者への自動追加から除外される（DB のユニーク制約 `[inquiryId, userId, deletedAt]` との整合性を保つため）
- 申請オーナーが除外された場合、初期ステータスは `UNASSIGNED` となる

### 5.2 申請に関連するお問い合わせ（実委側作成時）

- 実委側は申請の存在チェックのみ行い、初期対応ルール（自動担当者・閲覧者割り当て）は適用しない
- 作成者が実委側担当者となるため、ステータスは `IN_PROGRESS` で開始

### 5.3 申請に関連しないお問い合わせ

- 企画側作成の場合、ステータスは `UNASSIGNED` となる
...
- 実委側の担当者 / お問い合わせ管理者は、コメントを下書きとして保存できる
- 下書きは当該お問い合わせを閲覧可能な実委メンバー全員に表示される
- 下書きの正式送信（公開）と削除は、下書き作成者のみ実行できる
- 下書きを正式送信したとき、コメントの表示時刻は「下書き作成時刻」ではなく「送信操作時刻」を使用する
- 下書きの正式送信時には、お問い合わせの `updatedAt` も送信操作時刻で更新する
...
| POST | `/committee/inquiries/:inquiryId/comments/:commentId/publish` | 下書きコメントを正式送信 | 下書き作成者 |
| DELETE | `/committee/inquiries/:inquiryId/comments/:commentId` | コメント削除 | 下書き: 作成者のみ / 通常コメント: 管理者または作成者 |
...
- 下書きコメントに紐づく添付ファイルは、実委側の閲覧権限を持つユーザーのみアクセス可能（企画側担当者はアクセス不可）
...
- 一覧に表示する「○件の返信」は公開済みコメント件数（下書きは含めない）
...
- サイドバー: ステータス、担当者一覧、閲覧者設定（実委側のみ）、関連申請（実委側はリンク、企画側はテキスト表示）
- メイン: タイムライン（コメント + アクティビティ）
- 実委側の下書きコメントは、そのお問い合わせを閲覧可能な実委メンバー全員が確認できる
- 実委側の返信UIは「コメントを追加」「お問い合わせの下書き」タブを持つ
- 「お問い合わせの下書き」タブには下書き件数バッジを表示し、0件のときは表示しない
- 下書きの送信 / 削除ボタンは処理中にローディング表示し、重複操作を防ぐため無効化する
- 下書き保存成功時は成功トーストを表示する
...
- 企画側: 件名、内容、関連申請（配信された申請から選択）、共同担当者、添付ファイル
- 実委側: 件名、内容、関連申請（オーナーまたは共同編集者の申請から選択）、対象企画、企画側担当者（必須）、追加実委担当者、閲覧者、添付ファイル
- 送信中はボタンを無効化しローディング表示
- API エラー時はトースト通知を表示し、ダイアログは閉じない（申請データを保持）
